...
class Theme
{
	...
	public function __construct()
	{
		...
		$this->Form = new Form();
		$this->WP = new \Nongol\WordPress\Helper();
		/** Init Config */
		...
		$this->config->path =
			...
			'.php';
		$this->config->options = $this->WP->get_option('nongol_config');
	...
	public function loadModulesHooks()
	{
		...
		foreach (
			$this->Helper->getDirFiles(
				$this->path['framework_path'] . 'src/Helper/NongolModule'
			)
			as $module
		) {
	...
	private function loadHooks($dir)
	{
		...
		foreach ($controllers as $controller) {
			...
			$controller = '\\Nongol\\' . ucwords($dir) . '\\' . $name;
			...
			foreach ($controller->getHooks() as $hook) {
				$class = str_replace('Nongol\\WordPress\\Hook\\', '', get_class($hook));
				if (in_array(strtolower($class), $this->enableHooks)) {
					...
					$status = isset($this->config->options->nongol_hooks->$key)
						? $this->config->options->nongol_hooks->$key
	...
	public function setDefaultOption()
	{
		...
		if (empty($config) || !$config) {
			...
			$this->WP->update_option('nongol_config', (object) $config);
